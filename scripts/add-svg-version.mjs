...
export async function main() {
  ...
  for (const doc of docs) {
    ...
    // replace https://banglawiki.github.io/bangla-translate/assets/sponsorkit/sponsors.svg?v=xxx
    // to new hash
    const newContent = content.replace(
      /https:\/\/banglawiki\.github\.io\/bangla-translate\/assets\/sponsorkit\/sponsors\.svg\?v=[a-z0-9]+/g,
      `https://banglawiki.github.io/bangla-translate/assets/sponsorkit/sponsors.svg?v=${hash}`
    );
  ...
  for (const doc of contributorsDocs) {
    const content = await fs.readFile(doc, "utf8");
    // replace https://banglawiki.github.io/bangla-translate/assets/contributors/contributors.svg?v=xxxx
    // to new hash
    const newContent = content.replace(
      /https:\/\/banglawiki\.github\.io\/bangla-translate\/assets\/contributors\/contributors\.svg\?v=[a-z0-9]+/g,
      `https://banglawiki.github.io/bangla-translate/assets/contributors/contributors.svg?v=${contributorsHash}`
    );
    await fs.writeFile(doc, newContent);
  }
}
